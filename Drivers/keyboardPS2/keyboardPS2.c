...
#include "keyboardPS2stm32h7xx.h"
...
uint8_t ps2_kbd_getkey(uint8_t * kbd_key)
{
  ...
  while(1)
  {
    if(ps2_kbd_dataread(&ps2_kbd_s) == 0)
    {
      ...
      GPIOX_CLR(PS2_PIN_DEBUG_1);
      #endif
      return 0;                         /* nincs a billenty� pufferben semmi */
    }
    if(ps2_kbd_s == 0xE0)
    { /* k�t b�jtos karakter */
    ...
    { /* gomb felenged�s st�tusz */
      if(ps2_kbd_s == 0x12)
        ...
        state &= ~ST_KBDALTGR;
      state &= ~(ST_KBDBREAK | ST_KBDMODIFIER); /* BREAK/MODIFIER st�tusz ki */
    }
    else
    {                                   /* gomb lenyom�s vagy ism�tl�s */
      if(state & ST_KBDMODIFIER)
      {                                 /* k�t b�jtos (MODIFIER) gombok (INS, DEL, 4 nyil stb.) */
        if(ps2_kbd_s == 0x11)
        {
