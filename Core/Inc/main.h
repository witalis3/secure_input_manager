/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2024 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32h7xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define K1_BUTTON_Pin GPIO_PIN_3
#define K1_BUTTON_GPIO_Port GPIOE
#define SD_CS_Pin GPIO_PIN_0
#define SD_CS_GPIO_Port GPIOA
#define LED_Pin GPIO_PIN_1
#define LED_GPIO_Port GPIOA
#define KEYPAD_ROW_1_Pin GPIO_PIN_5
#define KEYPAD_ROW_1_GPIO_Port GPIOA
#define KEYPAD_ROW_2_Pin GPIO_PIN_7
#define KEYPAD_ROW_2_GPIO_Port GPIOA
#define KEYPAD_ROW_3_Pin GPIO_PIN_5
#define KEYPAD_ROW_3_GPIO_Port GPIOC
#define KEYPAD_ROW_4_Pin GPIO_PIN_1
#define KEYPAD_ROW_4_GPIO_Port GPIOB
#define KEYPAD_COL_1_Pin GPIO_PIN_7
#define KEYPAD_COL_1_GPIO_Port GPIOE
#define KEYPAD_COL_2_Pin GPIO_PIN_9
#define KEYPAD_COL_2_GPIO_Port GPIOE
#define KEYPAD_COL_3_Pin GPIO_PIN_11
#define KEYPAD_COL_3_GPIO_Port GPIOE
#define KEYPAD_COL_4_Pin GPIO_PIN_13
#define KEYPAD_COL_4_GPIO_Port GPIOE
#define CLK_Pin GPIO_PIN_8
#define CLK_GPIO_Port GPIOD
#define DIN_Pin GPIO_PIN_9
#define DIN_GPIO_Port GPIOD
#define DEBUG2_Pin GPIO_PIN_6
#define DEBUG2_GPIO_Port GPIOD
#define DEBUG1_Pin GPIO_PIN_1
#define DEBUG1_GPIO_Port GPIOE

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
